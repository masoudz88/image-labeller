...
(function(ModelSource) {
    ModelSource["DOWNLOAD"] = "DOWNLOAD";
    ModelSource["UPLOAD"] = "UPLOAD";
})(ModelSource || (ModelSource = {}));

var PretrainedModel;

(function(PretrainedModel) {
    PretrainedModel["YOLO_V5_N_COCO"] = "YOLO_V5_N_COCO";
    PretrainedModel["YOLO_V5_S_COCO"] = "YOLO_V5_S_COCO";
    PretrainedModel["YOLO_V5_M_COCO"] = "YOLO_V5_M_COCO";
})(PretrainedModel || (PretrainedModel = {}));

const PretrainedModelDataMap = {
...
const LoadYOLOv5ModelPopup = ({ updateActivePopupTypeAction, submitNewNotificationAction }) => {
    ...
    const onDrop = (accepted) => {
        const jsonFiles = accepted.filter((file) => file.name.endsWith('json'));
        const binFiles = accepted.filter((file) => file.name.endsWith('bin'));
        const txtFiles = accepted.filter((file) => file.name.endsWith('txt'));
    ...
    const onAccept = () => {
        ...
        const onFailure = () => {
            ...
            submitNewNotificationAction(
                NotificationUtil.createErrorNotification(NotificationsDataMap[notification])
            )
        }
        setIsLoading(true)
        if (modelSource === ModelSource.DOWNLOAD) {
            ...
            const config = { source: modelFiles, classNames: classNames.map((className) => className.name) }
    ...
    const changeModelSource = (source) => {
        ...
        return Object.entries(PretrainedModelDataMap).map(([key, value]) => {
            return (
                <div
                    ...
                    onClick={() => setSelectedPretrainedModel(key)}
                    key={key}>
    ...
    const renderMessage = () => {
        const uploadMessage = 'Drag and drop your own YOLOv5 model converted to tensorflow.js format and ' +
        ...
        const downloadMessage = 'Use one of ours pretrained YOLOv5 models to speed up annotation process.'
        return (
