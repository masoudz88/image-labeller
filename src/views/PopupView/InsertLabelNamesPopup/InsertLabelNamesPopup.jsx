...
const InsertLabelNamesPopup = (
    ...
    const validateEmptyLabelNames = () => {
        const emptyLabelNames = filter(labelNames, (labelName) => labelName.name === '');
        return emptyLabelNames.length === 0;
    };

    const validateNonUniqueLabelNames = () => {
        const uniqueLabelNames = uniq(labelNames.map((labelName) => labelName.name));
        return uniqueLabelNames.length === labelNames.length;
    };

    const callbackWithLabelNamesValidation = callback => {
        return () => {
    ...
    const deleteLabelNameCallback = (id) => {
    ...
    const changeLabelNameColorCallback = (id) => {
        const newLabelNames = labelNames.map((labelName) => {
    ...
    const labelInputs = labelNames.map((labelName) => {
        ...
        return (
            <div className='LabelEntry' key={labelName.id}>
                ...
                {projectType === ProjectType.OBJECT_DETECTION && enablePerClassColoration && <ColorSelectorView color={labelName.color} onClick={onChangeColorCallback} />}
                <ImageButton
                    image={'ico/trash.png'}
    ...
    const onCreateAcceptCallback = () => {
        const nonEmptyLabelNames = reject(labelNames, (labelName) => labelName.name.length === 0);
    ...
    const onUpdateAcceptCallback = () => {
        const nonEmptyLabelNames = reject(labelNames, (labelName) => labelName.name.length === 0);
        const missingIds = LabelUtil.labelNamesIdsDiff(LabelsSelector.getLabelNames(), nonEmptyLabelNames);
