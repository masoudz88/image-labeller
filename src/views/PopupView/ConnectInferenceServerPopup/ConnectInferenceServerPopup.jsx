...
const ConnectInferenceServerPopup = (
    {
        roboflowAPIDetails,
    ...
    const wrapServerOnClick = (newServerType) => {
        return () => {
            if (!InferenceServerDataMap[newServerType].isDisabled) {
                setCurrentServerType(newServerType)
            } else {
                submitNewNotificationAction(
                    NotificationUtil.createMessageNotification(NotificationsDataMap[Notification.UNSUPPORTED_INFERENCE_SERVER_MESSAGE])
                );
                return;
            }
        };
    ...
    const onAccept = () => {
        ...
        const onFailure = () => {
            submitNewNotificationAction(
                NotificationUtil.createErrorNotification(NotificationsDataMap[Notification.ROBOFLOW_INFERENCE_SERVER_ERROR])
            );
    ...
    const roboflowModelOnChangeCallback = (event) => {
        setRoboflowModel(event.target.value)
    }

    const roboflowKeyOnChangeCallback = (event) => {
    ...
    const renderSideMenuContent = () => {
        return Object.entries(InferenceServerDataMap).map(([serverType, serverData], index) => {
            return (
                <ImageButton
                    key={index}
                    ...
                    padding={20}
                    onClick={wrapServerOnClick(serverType)}
