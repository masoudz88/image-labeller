...
describe('ArrayUtil partition method', () => {
    it('should return empty PartitionResult if array is empty', () => {
        // when
        const result = ArrayUtil.partition([], (item) => true)
    ...
    it('should pass items with key name from list', () => {
        ...
        const predicate = (item) => nameList.includes(item.name)
        ...
        expect(result.pass.map((item) => item.name)).toEqual(['aaa', 'bbb']);
        expect(result.fail.map((item) => item.name)).toEqual(['aab', 'abb']);
    });
});
...
describe('ArrayUtil match method', () => {
    ...
    it(
        'should return correct array when number of keys and values is even',
        () => {
            // when
    ...
    it(
        'should return correct array when number of keys smaller than values',
        () => {
            // given
