...
describe('YOLOImporter filterFilesData method', () => {
    it('should return correct fileData partition', () => {
        ...
        const imagesData = imageFileNames.map((fileName) => getDummyImageData(fileName));
        ...
        const annotationFiles = annotationFileNames.map((fileName) => getDummyFileData(fileName));
        const labelFileNames = [
            'labels.txt'
        ];
        const labelFiles = labelFileNames.map((fileName) => getDummyFileData(fileName));
        const filesData = [...annotationFiles, ...labelFiles];
        ...
        // then
        const resultNames = result.annotationFiles.map((item) => item.name);
